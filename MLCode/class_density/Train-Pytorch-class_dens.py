...
def classification_prediction(dataloader,size,model,whole_dataset):
    ...
    df.to_csv(savepath+'predictions_class_density_bw_v_h_res_'+str(size)+'_'+str(myseed)+'_val_avon.csv',index=False)
...
def density_as_func_proba_density(csv_file,size_samp=10000):
    ...
    df = pd.DataFrame(dict)
    df.to_csv(savepath+'class_density_bw_v_h_res_proba_'+str(size)+'_'+str(size_samp)+'_'+str(myseed)+'.csv',index=False)
...
np.savetxt(savepath+method+'_'+dataname+'cm_val.txt',cm,fmt='%d',header=str_classes_names)

print('--> computing/saving classification outputs')
classification_prediction(val,size,model,whole_dataset)

csv_file=savepath+'predictions_class_density_bw_v_h_res_'+str(size)+'_'+str(myseed)+'_val_avon.csv'

...
echo "PERCO: dir=" $dir ",seed:"$seed ",py="$py" ,size:"$size ", size_samp:"$size_samp ", validation_split:"$validation_split ", batch_size:"$batch_size ", num_epochs:"$num_epochs
...
python $codedir/$py $seed $size $size_samp $validation_split $batch_size $num_epochs
